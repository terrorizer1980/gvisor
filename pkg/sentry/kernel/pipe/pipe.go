...
func (p *Pipe) writeLocked(count int64, f func(safemem.BlockSeq) (uint64, error)) (int64, error) {
	...
	if oldLen := int64(len(p.buf)); p.size+count > oldLen {
		// Allocate a new buffer.
		newLen := oldLen * 2
		if oldLen == 0 {
			newLen = 8 // arbitrary; sending individual integers across pipes is relatively common
		}
		for p.size+count > newLen {
			newLen *= 2
		}
		if newLen > p.max {
			newLen = p.max
