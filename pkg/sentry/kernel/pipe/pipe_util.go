...
func (p *Pipe) Read(ctx context.Context, dst usermem.IOSequence) (int64, error) {
	...
	if n > 0 {
		// This can't be called while holding MM locks.
...
func (p *Pipe) ReadToBlocks(dsts safemem.BlockSeq) (uint64, error) {
	p.mu.Lock()
	defer p.mu.Unlock()
	n, err := p.peekLocked(int64(dsts.NumBytes()), func(srcs safemem.BlockSeq) (uint64, error) {
		return safemem.CopySeq(dsts, srcs)
	})
	if n > 0 {
		p.consumeLocked(n)
		// Notify is called by Pipe.Read since we can't do so while holding MM
		// locks.
	}
	return uint64(n), err
}
...
func (p *Pipe) WriteTo(ctx context.Context, w io.Writer, count int64, dup bool) (int64, error) {
	p.mu.Lock()
	defer p.mu.Unlock()
	n, err := p.peekLocked(count, func(srcs safemem.BlockSeq) (uint64, error) {
...
func (p *Pipe) WriteFromBlocks(srcs safemem.BlockSeq) (uint64, error) {
	p.mu.Lock()
	defer p.mu.Unlock()
	n, err := p.writeLocked(int64(srcs.NumBytes()), func(dsts safemem.BlockSeq) (uint64, error) {
		return safemem.CopySeq(dsts, srcs)
	})
	return uint64(n), err
}
...
func (p *Pipe) ReadFrom(ctx context.Context, r io.Reader, count int64) (int64, error) {
	p.mu.Lock()
	defer p.mu.Unlock()
	n, err := p.writeLocked(count, func(dsts safemem.BlockSeq) (uint64, error) {
