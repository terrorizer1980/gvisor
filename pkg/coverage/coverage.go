...
func Symbolize(pc uint64) error {
	...
	printBlock(pc, file, block)
	return nil
}

// PrintAllBlocks prints all information about all blocks along with their
// corresponding synthetic PCs.
func PrintAllBlocks() {
	for fileNum, file := range globalData.files {
		for blockNum, block := range coverdata.Cover.Blocks[file] {
			printBlock(calculateSyntheticPC(fileNum, blockNum), file, block)
		}
	}
}
...
func syntheticPCToIndexes(pc uint64) (fileNum int, blockNum int) {
	return int(pc >> 16), int(pc & ((1 << 16) - 1))
}
...
func printBlock(pc uint64, file string, block testing.CoverBlock) {
	fmt.Printf("%#x\n", pc)
