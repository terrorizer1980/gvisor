...
func TestNeighborCacheEntry(t *testing.T) {
	...
	if _, _, err := neigh.entry(entry.Addr, "", linkRes, nil, nil); err != tcpip.ErrWouldBlock {
	...
	if _, _, err := neigh.entry(entry.Addr, "", linkRes, nil, nil); err != nil {
...
func (c *testContext) overflowCache(opts overflowOptions) error {
	...
	for i := opts.startAtEntryIndex; i < c.store.size(); i++ {
		...
		if _, _, err := c.neigh.entry(entry.Addr, "", c.linkRes, nil, nil); err != tcpip.ErrWouldBlock {
...
func TestNeighborCacheNotifiesWaker(t *testing.T) {
	...
	_, ch, err := neigh.entry(entry.Addr, "", linkRes, &w, func(linkAddr tcpip.LinkAddress, ok bool) {
		if !ok {
...
func TestNeighborCacheAddStaticEntryThenOverflow(t *testing.T) {
	...
	e, _, err := c.neigh.entry(entry.Addr, "", c.linkRes, nil, nil)
	if err != nil {
...
func TestNeighborCacheKeepFrequentlyUsed(t *testing.T) {
	...
	for i := 0; i < neighborCacheSize; i++ {
		...
		_, ch, err := neigh.entry(entry.Addr, "", linkRes, nil, func(linkAddr tcpip.LinkAddress, ok bool) {
			if !ok {
				t.Fatal("expected successful address resolution")
			}
			if linkAddr != entry.LinkAddr {
	...
	for i := neighborCacheSize; i < store.size(); i++ {
		...
		if i%(neighborCacheSize/2) == 0 {
			if _, _, err := neigh.entry(frequentlyUsedEntry.Addr, "", linkRes, nil, nil); err != nil {
	...
	for _, entry := range storeEntries {
		...
		for r := 0; r < concurrentProcesses; r++ {
			...
			go func(entry NeighborEntry) {
				defer wg.Done()
				if e, _, err := neigh.entry(entry.Addr, "", linkRes, nil, nil); err != nil && err != tcpip.ErrWouldBlock {
...
func TestNeighborCacheReplace(t *testing.T) {
	...
	{
		e, ch, err := neigh.entry(entry.Addr, "", linkRes, nil, func(linkAddr tcpip.LinkAddress, ok bool) {
	...
	{
		e, _, err := neigh.entry(entry.Addr, "", linkRes, nil, nil)
		if err != nil {
...
func TestNeighborCacheResolutionFailed(t *testing.T) {
	...
	// First, sanity check that resolution is working
	{
		...
		case <-ch:
		default:
			t.Fatalf("expected message over address resolution channel, want = %s", entry.LinkAddr)
		}
	}

	got, _, err := neigh.entry(entry.Addr, "", linkRes, nil, nil)
	if err != nil {
		t.Fatalf("unexpected error from neigh.entry(%s, '', _, nil, nil): %s", entry.Addr, err)
	}
	want := NeighborEntry{
...
func TestNeighborCacheRetryResolution(t *testing.T) {
	...
	{
		incompleteEntry, ch, err := neigh.entry(entry.Addr, "", linkRes, nil, nil)
		if err != tcpip.ErrWouldBlock {
			t.Fatalf("got neigh.entry(%s, '', _, _, nil) = %v, want = %s", entry.Addr, err, tcpip.ErrWouldBlock)
		}
		if incompleteEntry.State != Incomplete {
			t.Fatalf("got entry.State = %s, want = %s", incompleteEntry.State.String(), Incomplete.String())
		}
		clock.Advance(typicalLatency)
		...
		case <-ch:
			...
			reachableEntry, _, err := neigh.entry(entry.Addr, "", linkRes, nil, nil)
			if err != nil {
...
func TestNeighborCacheStaticResolution(t *testing.T) {
	...
	got, _, err := neigh.entry(testEntryBroadcastAddr, "", linkRes, nil, nil)
	if err != nil {
