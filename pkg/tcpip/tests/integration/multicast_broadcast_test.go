...
func TestUDPAddRemoveMembershipSocketOption(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			for _, specifyNICID := range boolArr {
				t.Run(fmt.Sprintf("SpecifyNICID=%t", specifyNICID), func(t *testing.T) {
					for _, specifyNICAddr := range boolArr {
						t.Run(fmt.Sprintf("SpecifyNICAddr=%t", specifyNICAddr), func(t *testing.T) {
							s := stack.New(stack.Options{
								NetworkProtocols:   []stack.NetworkProtocolFactory{ipv4.NewProtocol, ipv6.NewProtocol},
								TransportProtocols: []stack.TransportProtocolFactory{udp.NewProtocol},
							})
							...
							if !specifyNICID && !specifyNICAddr {
								s.SetRouteTable([]tcpip.Route{
									tcpip.Route{
										Destination: header.IPv6EmptySubnet,
										NIC:         nicID,
							...
							if gotPayload, _, err := ep.Read(nil); err != tcpip.ErrWouldBlock {
								t.Fatalf("got Read(nil) = (%x, _, %s), want = (nil, _, %s)", gotPayload, err, tcpip.ErrWouldBlock)
							}
						})
					}
				})
